...
class CookingSkill {
  public getSavedRecipes = async (req: Request, res: Response) => {
  ...
  public search = async (req: Request, res: Response) => {
    const hrstart = process.hrtime();
    const msg = req.query.msg;
    const temp_msg = msg.replace(/.*((masak)|(bikin)|(buat)|(resep))/g,"").trim();
    // temp_msg = msg.replace(/resep/g, "").trim();
      ...
      .then(async(html) => {
        ...
        for(let i=0;i<list_pilihan.length;i++){
          ...
          if(temp_msg === list_pilihan[i]){
            if(list_url[i].search('resep/resep')>-1) {
              return await this.detailResepTemplateA(list_url[i]);
            }
            else {
              return await this.detailResepTemplateB(list_url[i]);
            }
          }
  ...
  private detailResepTemplateB = async (url: string) => {
    let title = "detail-resep";
    const out = await rp(url)
      .then(async(html) => {
        var list_bahan: string[] = [];
        $('.ingredients-container', html).find('> ul > li > span').each(function (index, element) {
        ...
        $('.directions-container', html).find('> div > ol > li > span').each(function (index, element) {
            let step = $(element).text();
            list_langkah.push(step);
        });
  ...
  private detailResepTemplateA = async (url: string) => {
    let title = "detail-resep";

    const out = await rp(url)
      .then(async(html) => {
        var list_bahan: string[] = [];
        $('.ingredients-table', html).find('> tbody > tr').each(function (index, element) {
            let bahan = $('> td',element).last().text();
            // preprocessing untuk tts
            bahan = bahan.replace(/\n/g, "");
            bahan = bahan.replace(/[^a-z^A-z]\-[^a-z^A-z]/g," sebanyak ");
            bahan = bahan.replace(/\-/g," ");
